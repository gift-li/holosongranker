...
def Profile(request, id):
    ...
    # 取得搜尋條件
    order_query, date_query = get_profile_search_query(request)
...
def get_profile_search_query(request):
    order_query = 'weekly_view'
    ...
    if request.method == 'GET':
        ...
        if 'profile_view_select' not in request.session \
                or 'profile_date_select' not in request.session:
            request.session['profile_view_select'] = order_query
            request.session['profile_date_select'] = datetime.strftime(
                date_query, "%Y-%m-%d")
    ...
    if request.method == 'POST':
        ...
        request.session['profile_view_select'] = request.POST.get(
            'view_select')
        request.session['profile_date_select'] = request.POST.get(
            'date_select')

    # * 用Session賦值
    order_query = request.session['profile_view_select']
    date_query = datetime.strptime(
        request.session['profile_date_select'], "%Y-%m-%d").date()
...
def get_search_query(request, type):
    ...
    return order_query, date_query
    # if request.method == 'GET':
    #     order_query = request.GET.get('view', 'weekly_view')
